...
class Hanzoai::Test::PrimitiveModelTest < Minitest::Test
  A = Hanzoai::ArrayOf[-> { Integer }]
  H = Hanzoai::HashOf[-> { Integer }, nil?: true]
  ...
  def test_coerce
    cases = {
      [Hanzoai::Unknown, :a] => [{yes: 1}, :a],
      [NilClass, :a] => [{maybe: 1}, nil],
      [NilClass, nil] => [{yes: 1}, nil],
      [Hanzoai::BooleanModel, true] => [{yes: 1}, true],
      [Hanzoai::BooleanModel, "true"] => [{no: 1}, "true"],
      [Integer, 1] => [{yes: 1}, 1],
    ...
    cases.each do |lhs, rhs|
      ...
      assert_pattern do
        Hanzoai::Converter.coerce(target, input, state: state) => ^expect
  ...
  def test_dump
    cases = {
      [Hanzoai::Unknown, B.new(a: "one", b: B.new(a: 1.0))] => {a: "one", b: {a: 1}},
      ...
      [Hanzoai::BooleanModel, true] => true,
      [Hanzoai::BooleanModel, "true"] => "true",
      [Integer, "1"] => "1",
      [Float, 1] => 1,
      [String, "one"] => "one",
    ...
    cases.each do
      ...
      expect = _2
      assert_pattern do
        Hanzoai::Converter.dump(target, input) => ^expect
      end
    end
  end
  ...
  def test_coerce_errors
    ...
    cases.each do
      ...
      assert_raises(_2) do
        Hanzoai::Converter.coerce(target, input, state: state)
      end
    end
  end
end

class Hanzoai::Test::EnumModelTest < Minitest::Test
  module E1
    extend Hanzoai::Enum
...
class Hanzoai::Test::CollectionModelTest < Minitest::Test
  A1 = Hanzoai::ArrayOf[-> { Integer }]
  H1 = Hanzoai::HashOf[Integer]

  A2 = Hanzoai::ArrayOf[H1]
  H2 = Hanzoai::HashOf[-> { A1 }]

  A3 = Hanzoai::ArrayOf[Integer, nil?: true]
  H3 = Hanzoai::HashOf[Integer, nil?: true]
...
class Hanzoai::Test::BaseModelTest < Minitest::Test
  class M1 < Hanzoai::BaseModel
    required :a, Integer
  end
  ...
  def test_coerce
    ...
    cases.each do |lhs, rhs|
      ...
      state = {strictness: true, exactness: {yes: 0, no: 0, maybe: 0}, branched: 0}
      assert_pattern do
        coerced = Hanzoai::Converter.coerce(target, input, state: state)
        assert_equal(coerced, coerced)
        if coerced.is_a?(Hanzoai::BaseModel)
          coerced.to_h => ^expect
        else
          coerced => ^expect
        end
      ...
      _2.each do |accessor, expect|
        ...
        in Class if expect <= StandardError
          tap do
            ...
            flunk
          rescue Hanzoai::ConversionError => e
...
class Hanzoai::Test::UnionTest < Minitest::Test
  module U0
    extend Hanzoai::Union
  end
  ...
  class M4 < Hanzoai::BaseModel
    optional :recur, -> { U5 }
    required :a, Hanzoai::ArrayOf[-> { U5 }]
  end

  class M5 < Hanzoai::BaseModel
    optional :recur, -> { U5 }
    required :b, Hanzoai::ArrayOf[-> { U5 }]
  end
...
class Hanzoai::Test::BaseModelQoLTest < Minitest::Test
  module E1
    extend Hanzoai::Enum
  ...
  def test_equality
    cases = {
      [Hanzoai::Unknown, Hanzoai::Unknown] => true,
      [Hanzoai::BooleanModel, Hanzoai::BooleanModel] => true,
      [Hanzoai::Unknown, Hanzoai::BooleanModel] => false,
      [E1, E2] => true,
      [E1, E3] => false,
      [M1, M2] => false,
      [M1, M3] => true
