  ...
  class PooledNetRequester
    ...
    class << self
      ...
      def calibrate_socket_timeout(conn, deadline)
        timeout = deadline - HanzoAI::Util.monotonic_secs
      ...
      def build_request(request, &)
        ...
        in String
          ...
          req.body_stream = HanzoAI::Util::ReadIOAdapter.new(body, &)
        in StringIO
    ...
    private def with_pool(url, deadline:, &blk)
      origin = HanzoAI::Util.uri_origin(url)
      timeout = deadline - HanzoAI::Util.monotonic_secs
      pool =
    ...
    def execute(request)
      ...
      enum = Enumerator.new do |y|
        ...
        raise HanzoAI::APITimeoutError
      end

      conn, _, response = enum.next
      body = HanzoAI::Util.fused_enum(enum, external: true) do
        finished = true
        tap do
          enum.next
